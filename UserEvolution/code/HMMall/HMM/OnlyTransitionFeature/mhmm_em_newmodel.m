function [LL, prior, transmat, weightmat, mu, Sigma, mixmat] = ...
     mhmm_em_newmodel(data, prior, transmat, weightmat, features, mu, Sigma, mixmat, regCoeff,varargin)
...
[max_iter, thresh, verbose, cov_type,  adj_prior, adj_trans, adj_mix, adj_mu, adj_Sigma] = ...
    process_options(varargin, 'max_iter', 15, 'thresh', 1e-4, 'verbose', 1, ...
...
while (num_iter <= max_iter) & ~converged
  ...
  [loglik, exp_num_trans, exp_num_visits1, postmix, m, ip, op] = ...
      ess_mhmm(prior, transmat, weightmat, mixmat, mu, Sigma, data);
  ...
  if adj_trans
    ...
    exp_num_trans = mk_stochastic(exp_num_trans);
     [weightmat] = optimizeWeights(weightmat,transmat, exp_num_trans,regCoeff,features);
...
for ex=1:numex % each sequence
  ...
  if mix  %Ignore
	...
	fwdback(prior, weightmat, B, 'obslik2', B2, 'mixmat', mixmat);
  else
    B = mixgauss_prob(obs, mu, Sigma);
    [alpha, beta, gamma,  current_loglik, xi_summed] = fwdback_newmodel(prior, transmat, weightmat, B);
