function [alpha, beta, gamma, loglik, xi_summed, gamma2] = fwdback_newmodel(init_state_distrib, ...
   transmat, weightmat, obslik, varargin)
...
for t=T-1:-1:1
 ...
 if compute_xi
   ...
   xi_summed = xi_summed + normalise((alpha(:,t) * b'));
 end
 if compute_gamma2
   denom = obslik(:,t) + (obslik(:,t)==0); % replace 0s with 1s before dividing
   if iscell(mixmat)
