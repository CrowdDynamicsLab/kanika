function [LL, prior, transmat, weightmat, mu, Sigma] = ...
     mhmm_em_newmodel_withTime(data, prior, transmat, weightmat, features, mu, Sigma, regCoeff,varargin)
...
while (num_iter <= max_iter) & ~converged
  ...
  if adj_trans
    ...
    for j = 1:Q
        if(i ~= j)
            features(i,Q+k) = distance_unranked(mu,i,j);
            k = k+1;
        end
    end
    end
    ...
    %exp_num_trans = QXQXT
     [weightmat] = optimizeWeights_withTime(weightmat,exp_num_trans,regCoeff,features);
...
for ex=1:numex % each sequence
  ...
  [alpha, beta, gamma,  current_loglik, xi_summed] = fwdback_newmodel_withTime(prior, features, weightmat, B);
      
  loglik = loglik +  current_loglik; 
  if verbose, fprintf(1, 'll at ex %d = %f\n', ex, loglik); end
  
  T = size(xi_summed,3);
  exp_num_trans(:,:,1:T) =  exp_num_trans(:,:,1:T) + xi_summed; % sum(xi,3);
