function [loglik, errors] = mhmm_logprob(data, prior, transmat, mu, Sigma)
...
errors = [];
for m=1:ncases
  obslik = mixgauss_prob(data{m}, mu, Sigma, ones(Q,1));
  [alpha, beta, gamma, ll] = fwdback2(prior, transmat, obslik, 'fwd_only', 1);
  if ll==-inf
    errors = [errors m];
  end
  loglik = loglik + ll;
