...
class QADataset(Dataset):
    ...
    # reindex the pairs
    def change_ID(self):
        self.trainPairs['PairId_original'] = self.trainPairs['PairId']
        self.pairFeatures['PairId_original'] = self.pairFeatures['PairId']
        self.pairText['PairId_original'] = self.pairText['PairId']
        self.trainPairs_WFeatures['PairId_original'] = self.trainPairs_WFeatures['PairId']
    ...
    def getAdjList_clique(self):
        ...
        trainPairs["PairId"] = trainPairs["PairId"].apply(lambda x: int(x)-1)
        extra = trainPairs["PairId"].values[-1]
        Part = trainPairs.groupby("QuestionId")

        # group by QuestionId
        for index in Part:
            if index[0]!='0':# and len(index[1])==2:
        	    for i in combinations(index[1]["PairId"].tolist()+[extra],2):
