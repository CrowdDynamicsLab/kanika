...
class QADataset(Dataset):
    ...
    def filterDataset(self):
        ...
        for askerId, content in tempPairMerge.groupby("AskerId"):
            TempList = sorted([(i, j) for i,j in content.groupby("QuestionId")], key=lambda x: x[1]["QuestionAge"].values[0],reverse=True)
        ...
        if os.path.isfile(fname):
            ...
            with open(fname, 'r') as f:
               ...
               #print(self.trainPairs_WFeatures.keys())
                self.trainPairs_WFeatures['QuestionAge'] = self.trainPairs_WFeatures['QuestionId'].apply(lambda x: int(questionAgeDict[x]))
               ...
               ### change answerAge
                ...
                self.trainPairs_WFeatures['AnswerAge'] = self.trainPairs_WFeatures['CommentId'].apply(lambda x: int(answerAgeDict[x]))
    ...
    def getAdjList_Similarity2(self):
        ...
        file = os.path.join("/home/knarang2/stackexchange_adj_data/" + self.dataset + "/"+ 'edges_' + 'arrival' + '.pkl')
        if os.path.exists(file):
            edges = pickle.load(open(file,"r"))
            print ("similarity2:", len(edges))
            return edges
        ...
        for i,j in self.trainPairs_WFeatures.groupby('QuestionId'):
            ...
            self.trainPairs_WFeatures.at[self.trainPairs_WFeatures['QuestionId']==i,"ArrivalRank"] = ss.rankdata(temp)
    ...
    def getAdjList_Similarity3(self,QuestionList):
        ...
        #with open("/home/knarang2/stackexchange_adj_data/" + self.dataset + "/"+ 'edges_' + 'ts' + '.pkl', 'w') as f:
        #    pickle.dump( edges, f)
    ...
    def getAdjList_ContentSimilarity(self):
        ...
        file = os.path.join("/home/knarang2/stackexchange_adj_data/" + self.dataset + "/"+ 'edges_' + 'content' + '.pkl')
        if os.path.exists(file):
            edges = pickle.load(open(file,"r"))
        ...
        def getCosine(embeddings):
            ...
            for i in range(len(embeddings)):
                cosine[i] = sklearn.metrics.pairwise.cosine_similarity(embeddings[i,:50].reshape(1,-1), embeddings[i,50:].reshape(1,-1))[0]
    ...
    def getAdjList_clique(self):
        ...
        file = os.path.join("/home/knarang2/stackexchange_adj_data/", self.dataset, 'edges_clique.pkl')
        if os.path.exists(file):
            edges = pickle.load(open(file,"r"))
            print ("clique:", len(edges))
            return edges
        ...
        directory = "/home/knarang2/stackexchange_adj_data/" + self.dataset
