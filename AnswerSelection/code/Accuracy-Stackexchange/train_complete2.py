...
#Pairs_train, Pairs_test, Y_train, Y_test = train_test_split(X[X['Label']==1].loc[:,'QuestionId'].drop_duplicates(), X[X['Label']==1].loc[:,'QuestionId'].drop_duplicates(), test_size=0, random_state=1234)
trainset = torch.utils.data.TensorDataset(torch.LongTensor(X[X['Label']==1].loc[:,'QuestionId'].drop_duplicates().values), torch.torch.LongTensor(X[X['Label']==1].loc[:,'QuestionId'].drop_duplicates().values))
testset = torch.utils.data.TensorDataset(torch.LongTensor(X[X['Label']==0].loc[:,'QuestionId'].drop_duplicates().values), torch.torch.LongTensor(X[X['Label']==0].loc[:,'QuestionId'].drop_duplicates().values))
...
try:
    for epoch in range(1, args.num_epochs + 1):
        ...
        if epoch > 0:
            ...
            currcorrect, degrees = evaluate_line4(gcn_model, test_loader, Adj, X_Tags_Feature, rowsum, X)
        ...
        if epoch == args.num_epochs:
            ...
            for i, (X_test, Y_test) in enumerate(test_loader):
                ...
                X_test1 = torch.tensor(X[X["QuestionId"].isin(X_test)]['PairId'].values)
                Y_test1 = torch.tensor(X[X["QuestionId"].isin(X_test)]['Credible'].values)
                Q1 = torch.tensor(X[X["QuestionId"].isin(X_test)]['QScore'].values)
                A1 = torch.tensor(X[X["QuestionId"].isin(X_test)]['AScore'].values)
                QId1 = torch.tensor(X[X["QuestionId"].isin(X_test)]['QuestionId'].values)
                gcn_model.eval()
                user_gcn_embed = gcn_model(X_Tags_Feature, Adj)
                user_gcn_embed.squeeze_()
            ...
            data['QScore'] = Q_list
            data['AScore'] = A_list
            data['Credible'] = y_list
            data['Predict'] = predict_list
            data.to_csv('/home/chaoqi/Desktop/1.csv')
            """
            # build dictionary
