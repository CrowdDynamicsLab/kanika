...
try:
    ...
    for Pairs_train, Pairs_test in kf.split(Dataset):
        ...
        gcn_model = GCN_adaboost_content(nFeat, nHid1, nHid2, nHid3, nHid4, nComm).to(device)
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            for i, (X_train, Y_train) in enumerate(train_loader):
                ...
                sim1, sim2, sim3, part2_dense = gcn_model(X_Tags_Feature, Adj, Adj2, Adj3, Adj4, Adj5, Adj6, Target, X_train)
                ...
                if epoch%2==1:
                    loss = torch.add(torch.add(torch.sum(torch.exp(-torch.mul(sim1[X_train],Target[X_train]))),torch.sum(torch.exp(-torch.mul(sim2[X_train],Target[X_train])))), torch.sum(torch.exp(-torch.mul(sim3[X_train],Target[X_train]))))
                    sim2 = Variable(sim2, requires_grad=False)
                    loss += criterion(sim1[X_train],sim2[X_train])+ criterion(sim3[X_train],sim2[X_train]) + lambda1 * torch.norm(torch.cat((all_params_gc4,all_params_gc5,all_params_gc6,all_params_densesim)), 1)+lambda2 * torch.norm(torch.cat((all_params_gc4,all_params_gc5,all_params_gc6,all_params_densesim)), 2)
                    loss += criterion(sim1[X_train],sim3[X_train])
            ...
            if epoch > 0:
                ...
                currcorrect= evaluate_adaboost_content(gcn_model, test_loader, Adj, Adj2, Adj3, Adj4, Adj5, Adj6, X_Tags_Feature, rowsum, X, epoch, Target)
                ...
                print "best accuracy @"+ str(count) +" is", best_accuracy
