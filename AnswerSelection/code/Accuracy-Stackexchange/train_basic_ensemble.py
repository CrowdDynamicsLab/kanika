...
from test import evaluate_line_decay_rep1
import torch.nn.functional as F
from utils import getPostContexts
from sklearn import preprocessing
from torch.autograd import Variable
from model import GCN_decay_rep1
...
gcn_model = GCN_decay_rep1(nFeat, nHid1, nHid2, nHid3, nHid4, nComm)
...
try:
    for epoch in range(1, args.num_epochs + 1):
        ...
        for i, (X_train, Y_train) in enumerate(train_loader):
            ...
            user_gcn_embed,user_gcn_embed2,user_gcn_embed3,user_gcn_embed4,user_gcn_embed5 = gcn_model(X_Tags_Feature, Adj, Adj2, Adj3, Adj4, Adj5)
            ...
            predicted5 = user_gcn_embed5.squeeze()[X_train]>0
            train_acc5+= (Y_train.numpy()==predicted5.numpy()).sum()


            Y_train = Variable(Y_train, requires_grad=False)

            loss = decay*torch.sum(torch.exp(-torch.mul(user_gcn_embed[X_train],Target[X_train])))
            loss = torch.add(loss,decay*torch.sum(torch.exp(-torch.mul(user_gcn_embed2[X_train],Target[X_train]))))
            loss = torch.add(loss,decay*torch.sum(torch.exp(-torch.mul(user_gcn_embed3[X_train],Target[X_train]))))
            loss = torch.add(loss,decay*torch.sum(torch.exp(-torch.mul(user_gcn_embed4[X_train],Target[X_train]))))
            loss = torch.add(loss,torch.sum(torch.exp(-torch.mul(user_gcn_embed5[X_train],Target[X_train]))))
        ...
        if epoch > 0:
            ...
            currcorrect,currcorrect2,currcorrect3,currcorrect4,currcorrect5= evaluate_line_decay_rep1(gcn_model, test_loader, Adj, Adj2, Adj3, Adj4, Adj5, X_Tags_Feature, rowsum, X, epoch)

            print "Epoch ", epoch, "'s accuracy is", currcorrect, currcorrect2, currcorrect3, currcorrect4, currcorrect5
            ...
            print "Train Accuracy", train_acc, train_acc2, train_acc3, train_acc4, train_acc5
            print args.root
